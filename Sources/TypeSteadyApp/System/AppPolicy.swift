...
// D7 (проверено, код не менялся): AppPolicy() инстанцируется трижды — в InputCoordinator,
// CorrectionCoordinator и DetectionEngine. Это value type без изменяемого состояния экземпляра:
// единственные тяжёлые части (NSRegularExpression) уже вынесены в `static let` выше (B5) и
// разделяются между ВСЕМИ экземплярами автоматически, а не пересобираются на каждый доступ.
// Экземплярные свойства ниже — массивы/множества строковых констант, дешёвые в построении и
// не удерживающие ресурсов. Общий синглтон-экземпляр не даст измеримой экономии, а лишний
// параметр внедрения зависимости в трёх местах — накладные расходы без выгоды. Оставлено как
// есть намеренно.
struct AppPolicy {
