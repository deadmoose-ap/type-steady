...
let langSwitcherEventMarker: Int64 = 0x4C_53_57_54
...
final class InputEventTap: @unchecked Sendable {
    ...
    func start() -> Bool {
        ...
        guard let eventTap = CGEvent.tapCreate(
            ...
            callback: langSwitcherEventTapCallback,
            userInfo: Unmanaged.passUnretained(self).toOpaque()
        ) else { return false }

        tap = eventTap
        let worker = Thread { [weak self] in self?.runEventLoop(eventTap) }
        worker.name = "LangSwitcher.InputEventTap"
    ...
    fileprivate func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if event.getIntegerValueField(.eventSourceUserData) == langSwitcherEventMarker {
            return Unmanaged.passUnretained(event)
        }
...
private func langSwitcherEventTapCallback(
