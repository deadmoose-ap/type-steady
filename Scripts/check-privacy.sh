...
if grep -rEn "$FORBIDDEN_PATTERN" Sources Package.swift; then
    echo "Privacy check failed: forbidden API or dependency found." >&2
    exit 1
fi

if grep -rEn 'logger\.(debug|info|notice|warning|error|critical).*\b(word|text|selection|keyCode)\b' Sources; then
