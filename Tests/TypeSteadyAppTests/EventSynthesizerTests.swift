...
struct EventSynthesizerTests {
    ...
    @Test func chunksBMPTextAtRequestedBoundary() {
        let chunks = EventSynthesizer.chunkUTF16("abcdef", maximumCodeUnits: 2)
        #expect(chunks.map(\.count) == [2, 2, 2])
    }

    @Test func keepsSurrogatePairTogetherAtSmallestSupportedLimit() {
        let chunks = EventSynthesizer.chunkUTF16("🙂🙂", maximumCodeUnits: 2)
        #expect(chunks.map(\.count) == [2, 2])
    }
}
